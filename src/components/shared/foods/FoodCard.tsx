...
export const FoodCard: React.FC<IFood> = ({ foodId, name, amount, quantity, image }) => {
  ...
  return (
    <>
      <div className="relative flex items-center bg-muted/40 rounded-2xl border p-3">
        <img
          src={image}
          alt={name}
        ...
        <div className="flex-1 ml-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              className="cursor-pointer border w-8 h-8 flex items-center justify-center rounded-full bg-muted hover:bg-muted-foreground/10 transition"
              aria-label="Edit"
              type="button"
      ...
      <FoodFormDialog
        ...
        mode="edit"
        initialValues={{ foodId, name, amount, quantity, image }}
      />
    </>
  );
};